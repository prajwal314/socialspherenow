...
export default function Home() {
  ...
  const [connectingTo, setConnectingTo] = useState(null) // Track which user we're connecting to
  const [viewingProfileUserId, setViewingProfileUserId] = useState(null) // Track profile modal
  ...
  const userEventAttendance = useQuery(
    ...
    selectedEvent && user?.id ? { eventId: selectedEvent, userId: user.id } : 'skip'
  )
  ...
  const handleJoinEvent = async (status = 'going') => {
  ...
  const handleConnectWithUser = async (search) => {
    ...
    try {
      await createRequest({
        ...
        intent: partnerActivityLabels[search.activityType]?.label || search.activityType,
      })
    } catch (error) {
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      const dateTime = new Date(`${eventForm.date}T${eventForm.time}`).getTime()
      const peopleNeeded = eventForm.peopleNeeded ? parseInt(eventForm.peopleNeeded, 10) : undefined
      ...
      setEventForm({ activity: '', title: '', description: '', date: '', time: '', peopleNeeded: '' })
  ...
  const isFormValid = eventForm.activity && eventForm.title && eventForm.date && eventForm.time
      ...
      <main className="max-w-6xl mx-auto px-4 pt-20 pb-24">
        ...
        <header className="relative mb-8">
          ...
          <div className="relative">
            ...
            <p className="text-gray-400 mt-1">Discover events that match your vibe</p>
          </div>
        </header>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white">{request.senderName}</p>
                      <p className="text-sm text-gray-400 mt-0.5">
                        wants to connect for{' '}
                        <span className="text-purple-300 capitalize">{request.intent || request.activity}</span>
                      </p>
                    </div>
                  </div>
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-semibold text-white">Suggested Companions</h2>
          ...
          {activeSearches && activeSearches.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {activeSearches.slice(0, 6).map((search) => {
                const activityInfo = partnerActivityLabels[search.activityType] || { 
                  label: search.activityType, 
                  icon: '🎯' 
                }
                const alreadySent = hasAlreadySentRequest(search.userId)
                const isFull = search.peopleNeeded && search.spotsLeft === 0
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      <button
                        onClick={() => setViewingProfileUserId(search.userId)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500">
                          {search.userDetails?.personalityType && (
                            <span className="capitalize">{search.userDetails.personalityType}</span>
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      {search.preferences && Object.keys(search.preferences).length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(search.preferences).slice(0, 3).map(([key, value]) => (
                            <span 
                              key={key}
                          ...
                          {Object.keys(search.preferences).length > 3 && (
                            <span className="px-2 py-1 rounded-lg bg-white/5 text-xs text-gray-500">
                              +{Object.keys(search.preferences).length - 3} more
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    {/* Connect Button */}
                    <button
                      onClick={() => !alreadySent && !isFull && handleConnectWithUser(search)}
                      disabled={alreadySent || connectingTo === search._id || isFull}
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl bg-[#1e1e2e] border border-white/5">
              ...
              <p className="text-gray-400 text-sm">No one is looking for partners right now</p>
              <p className="text-gray-500 text-xs mt-1">Check back later or start your own search!</p>
            </div>
          )}
        </section>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-400">{event.creatorName}</span>
                      </div>
                      <button 
        ...
        <section className="mb-8">
          ...
          {communities && communities.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4 mb-3">
                      <button
                        onClick={() => isJoined && setViewingCommunityId(community._id)}
                      ...
                      >
                        {community.imageUrl ? (
                          <img src={community.imageUrl} alt="" className="w-full h-full rounded-xl object-cover" />
                        ) : (
                          '👥'
                        )}
                      </button>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl bg-[#1e1e2e] border border-white/5">
              ...
              <p className="text-gray-400 text-sm">No community suggestions yet</p>
            </div>
          )}
        </section>
      </main>
        ...
        >
          <div className="min-h-full flex items-center justify-center p-4 py-8">
            ...
            >
              ...
              <div>
                ...
                {eventImagePreview ? (
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-500">Add event image</span>
                  </button>
                )}
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">What is this event about?</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">
                  How many people do you need? <span className="text-gray-500">(optional)</span>
                </label>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <input
                      ...
                      onChange={(e) => setEventForm({ ...eventForm, peopleNeeded: e.target.value })}
                  ...
                  {eventForm.peopleNeeded && (
                    <div className="px-4 py-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
                      <span className="text-purple-300 text-sm font-medium">
                        {eventForm.peopleNeeded} {parseInt(eventForm.peopleNeeded) === 1 ? 'person' : 'people'}
                      </span>
                    </div>
                  )}
                </div>
                <p className="text-xs text-gray-500 mt-2">Event will auto-close when all spots are filled</p>
              </div>
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-white/5">
                      ...
                      <div>
                        ...
                        <p className="text-white font-medium">{formatDateTime(eventDetails.dateTime)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-white/5">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center text-white font-semibold">
                        {eventDetails.creator?.profileImageUrl ? (
                          <img src={eventDetails.creator.profileImageUrl} alt="" className="w-full h-full rounded-full object-cover" />
                        ) : (
                          eventDetails.creatorName?.charAt(0)?.toUpperCase() || '?'
                        )}
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">Hosted by</p>
                        <p className="text-white font-medium">{eventDetails.creatorName}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-white/5">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          eventDetails.peopleNeeded && eventDetails.spotsLeft === 0
                            ? 'bg-red-500/20 text-red-400'
                            : eventDetails.peopleNeeded && eventDetails.spotsLeft <= 2
                            ? 'bg-orange-500/20 text-orange-400'
                            : 'bg-green-500/20 text-green-400'
                        }`}>
                      ...
                      {/* Spots left message */}
                      {eventDetails.peopleNeeded && eventDetails.spotsLeft !== undefined && (
                        <div className={`mb-3 px-3 py-2 rounded-lg text-xs font-medium ${
                      ...
                      {eventDetails.attendees && eventDetails.attendees.length > 0 ? (
                        <div className="flex items-center gap-2">
                          <div className="flex -space-x-2">
                            {eventDetails.attendees.slice(0, 5).map((attendee, idx) => (
                              <div 
                                key={idx}
                          ...
                          <span className="text-sm text-gray-400">
                            {eventDetails.attendees.slice(0, 2).map(a => a.userName).join(', ')}
                            {eventDetails.attendees.length > 2 && ` and ${eventDetails.attendees.length - 2} more`}
                          </span>
                        </div>
                      ) : (
                    ...
                    {eventDetails.creatorId !== user?.id ? (
                      ...
                      <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20 text-center">
                        <p className="text-purple-300 text-sm">This is your event</p>
                      </div>
                    )}
                  </div>
                </>
