...
const personalityLabels = {
  introvert: { label: 'Introvert', icon: '🌙', description: 'Prefers quiet, meaningful connections' },
  extrovert: { label: 'Extrovert', icon: '☀️', description: 'Energized by social interactions' },
  ambivert: { label: 'Ambivert', icon: '🌗', description: 'Best of both worlds' },
}

export default function UserProfileModal({ targetUserId, currentUserId, onClose }) {
        ...
        >
          {profile === undefined ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-400 text-sm">This user profile is not available.</p>
              <button
                onClick={onClose}
              ...
              <div className="pt-16 pb-6 px-6">
                ...
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-bold text-white">
                    {profile.firstName}
                    {profile.isConnected && profile.lastName && ` ${profile.lastName}`}
                  </h2>
                  
                  {profile.personalityType && (
                    <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/10 text-purple-300 text-sm">
                      <span>{personalityLabels[profile.personalityType]?.icon || '🌟'}</span>
                      <span className="capitalize">{profile.personalityType}</span>
                    </div>
                  )}
                ...
                {profile.stats && (
                  <div className="flex items-center justify-center gap-6 mb-6">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-white">{profile.stats.connections}</p>
                    ...
                    {profile.isConnected && (
                      <>
                        ...
                        <div className="text-center">
                          <p className="text-2xl font-bold text-white">{profile.stats.eventsCreated}</p>
                          <p className="text-xs text-gray-500">Events</p>
                        </div>
                        <div className="w-px h-8 bg-white/10" />
                        <div className="text-center">
                          <p className="text-2xl font-bold text-white">{profile.stats.communitiesJoined}</p>
                  ...
                  <div className="space-y-4">
                    ...
                    {profile.intents && profile.intents.length > 0 && (
                      <div className="p-4 rounded-xl bg-white/5">
                        <h4 className="text-sm font-medium text-gray-400 mb-3">Looking for</h4>
                        <div className="flex flex-wrap gap-2">
                          {profile.intents.map((intent) => {
                            const info = intentLabels[intent] || { label: intent, icon: '🎯' }
                            return (
                              <span 
                                key={intent}
                    ...
                    {profile.activities && profile.activities.length > 0 && (
                      <div className="p-4 rounded-xl bg-white/5">
                        <h4 className="text-sm font-medium text-gray-400 mb-3">Interested in</h4>
                        <div className="flex flex-wrap gap-2">
                          {profile.activities.map((activity) => {
                            const info = activityLabels[activity] || { label: activity, icon: '🎯' }
                            return (
                              <span 
                                key={activity}
                    ...
                    {profile.availability && profile.availability.length > 0 && (
                      <div className="p-4 rounded-xl bg-white/5">
                        <h4 className="text-sm font-medium text-gray-400 mb-3">Usually available</h4>
                        <div className="flex flex-wrap gap-2">
                          {profile.availability.map((time) => {
                            const info = availabilityLabels[time] || { label: time, icon: '📅' }
                            return (
                              <span 
                                key={time}
                    ...
                    {profile.comfortPreference && (
                      <div className="p-4 rounded-xl bg-white/5">
                        <h4 className="text-sm font-medium text-gray-400 mb-2">Comfort preference</h4>
                        <p className="text-white capitalize">
                          {profile.comfortPreference === 'one-on-one' && '1-on-1 meetings'}
                          {profile.comfortPreference === 'small-group' && 'Small groups (2-4)'}
                          {profile.comfortPreference === 'any-size' && 'Any group size'}
                          {!['one-on-one', 'small-group', 'any-size'].includes(profile.comfortPreference) && profile.comfortPreference.replace(/-/g, ' ')}
                        </p>
                      </div>
                    )}
                ...
                {!profile.isConnected && (
                  <div className="p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-center">
                    ...
                    <p className="text-yellow-300 text-sm font-medium mb-1">Limited Profile</p>
                    <p className="text-yellow-400/70 text-xs">
                      Connect with {profile.firstName} to see their full profile
                    </p>
                  </div>
                )}
              </div>
