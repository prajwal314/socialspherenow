...
export default function CreateCommunityModal({ isOpen, onClose, userId }) {
  ...
  const canCreateMore = createdCount !== undefined && createdCount < MAX_COMMUNITIES
  ...
  const isStep1Valid = form.category && (form.category !== 'other' || form.customCategory.trim())
  ...
  return (
    ...
    >
      <div className="min-h-full flex items-center justify-center p-4 py-8">
        <div
          className="w-full max-w-lg rounded-3xl bg-[#1e1e2e] border border-white/10 shadow-2xl"
          ...
          {!canCreateMore && (
            <div className="px-6 py-4 bg-red-500/10 border-b border-red-500/20">
              <p className="text-red-400 text-sm">
                You have reached the maximum limit of {MAX_COMMUNITIES} communities. You cannot create more.
              </p>
            </div>
          )}
          ...
          {canCreateMore && remainingSlots <= 2 && (
            <div className="px-6 py-3 bg-orange-500/10 border-b border-orange-500/20">
              <p className="text-orange-400 text-sm">
                You can create {remainingSlots} more {remainingSlots === 1 ? 'community' : 'communities'}.
              </p>
            </div>
          )}
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-[300px] overflow-y-auto pr-1">
                    {COMMUNITY_CATEGORIES.map((cat) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-300 line-clamp-1">{cat.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500/20 to-purple-500/20 flex items-center justify-center text-lg">
                    {COMMUNITY_CATEGORIES.find((c) => c.id === form.category)?.icon || '✨'}
                  </div>
                  <div>
                    ...
                    <p className="text-white font-medium">
                      {form.category === 'other'
                        ? form.customCategory
                        : COMMUNITY_CATEGORIES.find((c) => c.id === form.category)?.label}
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">
                    Community Image <span className="text-gray-500">(optional)</span>
                  </label>
                  {imagePreview ? (
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-500">Add community image</span>
                    </button>
                  )}
                  <input
                    ref={imageInputRef}
