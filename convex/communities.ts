	...
	handler: async (ctx, args) => {
		...
		const filtered = allCommunities
			.filter((community) => {
				...
				const notOwnCommunity = args.excludeUserId
					? community.creatorId !== args.excludeUserId
					: true;
		...
		if (membership.role === "admin") {
			...
			if (allAdmins.length <= 1) {
				return {
					success: false,
					message:
						"You are the only admin. Please make someone else an admin before leaving.",
				};
			}
		}
		...
		const membersWithDetails = await Promise.all(
			memberships.map(async (membership) => {
				...
				if (user?.profileImageId) {
					profileImageUrl =
						(await ctx.storage.getUrl(user.profileImageId)) ?? profileImageUrl;
				}
