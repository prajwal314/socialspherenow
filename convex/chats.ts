...
export const getChatWithDetails = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (chat.type === "direct" && chat.participantIds) {
      ...
      if (otherUserId) {
        ...
        if (otherUser?.profileImageId) {
          profileImageUrl = await ctx.storage.getUrl(otherUser.profileImageId) || undefined;
        }
    ...
    if (chat.type === "event" && chat.eventId) {
      ...
      const userMembership = chatMembersList.find((m) => m.userId === args.currentUserId);
      const isAdmin = userMembership?.role === "admin" || chat.adminId === args.currentUserId;
...
export const getUserChatsWithDetails = query({
  ...
  handler: async (ctx, args) => {
    ...
    const chatSourceMap: Record<string, { activity: string; eventId?: string; activitySearchId?: string }> = {};
    ...
    const enrichedDirectChats = await Promise.all(
      directChats.map(async (chat) => {
        ...
        if (otherUserId) {
          ...
          if (otherUser?.profileImageId) {
            displayImage = await ctx.storage.getUrl(otherUser.profileImageId) || undefined;
          }
        ...
        return { ...chat, displayName: "Unknown", displayImage: null, connectionSource };
      })
    );
    ...
    const enrichedCommunityChats = await Promise.all(
      communityChats.map(async (chat) => {
        ...
        return { ...chat, displayName: "Community", displayImage: null, connectionSource: null };
      })
    );
    ...
    const enrichedGroupChats = await Promise.all(
      groupChats.map(async (chat) => {
        ...
        // Check if current user is admin
        const userMembership = userChatMemberships.find((m) => m.chatId === chat._id);
        const isAdmin = userMembership?.role === "admin" || chat.adminId === args.userId;
        ...
        if (chat.type === "activity" && chat.activitySearchId) {
          ...
          return {
            ...chat,
            displayName: chat.name || `${activitySearch?.activityType || "Activity"} Group`,
