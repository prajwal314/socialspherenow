			...
			if (activitySearch?.isActive) {
				...
				if (peopleNeeded > 2) {
					const activityChat = await ctx.db
						.query("chats")
						.withIndex("by_activity_search", (q) =>
							q.eq("activitySearchId", request.activitySearchId),
						)
						.first();
					...
					if (activityChat) {
						...
						const existingMember = await ctx.db
							...
							.withIndex("by_chat_and_user", (q) =>
								q
									.eq("chatId", activityChat._id)
									.eq("userId", request.receiverId),
							)
							.first();
