...
export const getAllLiveEventsExcludingUser = query({
  ...
  handler: async (ctx, args) => {
    ...
    const filteredEvents = allLiveEvents.filter((event) => event.creatorId !== args.userId);
    ...
    const eventsWithDetails = await Promise.all(
      filteredEvents.map(async (event) => {
        ...
        const spotsLeft = event.peopleNeeded ? event.peopleNeeded - goingCount : null;
    ...
    if (event.peopleNeeded && event.peopleNeeded > 2) {
      ...
      if (eventChat) {
        ...
        if (!existingMember) {
          ...
          const updatedParticipants = [...(eventChat.participantIds || []), args.userId];
