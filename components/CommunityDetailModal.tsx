...
export default function CommunityDetailModal({
	...
	const [activeTab, setActiveTab] = useState<"about" | "members" | "groups">(
		"about",
	);
	...
	const getCategoryInfo = (categoryId: string) => {
		return (
			COMMUNITY_CATEGORIES.find((c) => c.id === categoryId) || {
				label: categoryId,
				icon: "✨",
			}
		);
	...
	const handleCreateGroup = async (e: FormEvent) => {
		e.preventDefault();
		if (!groupForm.name.trim() || !communityId || !userId) return;
	...
	const handleJoinGroup = async (groupId: Id<"communityGroups">) => {
		if (!userId) return;
		try {
	...
	const handleLeaveGroup = async (groupId: Id<"communityGroups">) => {
		if (!userId) return;
		try {
	...
	const handleDeleteGroup = async (groupId: Id<"communityGroups">) => {
		if (!userId) return;
		if (
			!confirm(
							...
							{communityDetails && (
								<div className="absolute -bottom-4 left-4">
									<span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#1e1e2e] border border-white/10 text-sm">
										<span>
											{getCategoryInfo(communityDetails.category).icon}
										</span>
							...
							{communityDetails === undefined ? (
								...
								<div className="p-6 pt-8">
									...
									<div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
										...
										<span>
											Created {formatDate(communityDetails.createdAt)}
										</span>
									</div>

									{/* Tabs */}
									<div className="flex gap-2 mb-4 border-b border-white/10 pb-2">
										{(["about", "members", "groups"] as const).map((tab) => (
											<button
												type="button"
												key={tab}
												...
												<div className="flex gap-3">
													<button
														...
														onClick={() => {
															...
															setImageFile(null);
															setImagePreview(
																communityDetails.imageUrl || null,
															);
														}}
													...
													>
														...
														{isAdmin && member.userId !== userId && (
															<div>
																{member.role === "admin" ? (
																	member.userId !==
																		communityDetails.creatorId && (
																		<button
																			type="button"
																			onClick={() =>
																				handleRemoveAdmin(member.userId)
																			}
												...
												<div className="text-center py-8">
													...
													{isAdmin && (
														<p className="text-gray-600 text-xs mt-1">
															Create a group to organize discussions
														</p>
													)}
												</div>
											)}
