...
// Activity type for predefined activities
interface Activity {
	id: string;
	label: string;
	icon: string;
...
// Question type for activity modal
interface Question {
	id: string;
	label: string;
	type: "single";
...
// Activity questions map type
type ActivityQuestionsMap = Record<string, Question[]>;

// Match result from backend
interface MatchResult {
	searchId: Id<"activitySearches">;
	matchesFound: number;
	requestsSent: number;
}

// Current user type from query
...
const activityQuestions: ActivityQuestionsMap = {
	// 🏠 Find Roommate
	roommate: [
		{
...
export default function Explore() {
	...
	const allCommunities = useQuery(api.communities.getAllCommunities) as
		| Community[]
		| undefined
		| null;
	...
	// Mutations
	const createActivitySearchAndMatch = useMutation(
		api.activitySearches.createActivitySearchAndMatch,
	);
	...
	const filteredActivities = predefinedActivities.filter(
		(activity) =>
	...
	const sortedActivities = [...filteredActivities].sort((a, b) => {
		const aIsPreferred =
			currentUser?.activities?.includes(a.id) ||
			currentUser?.intents?.some((intent) => a.id.includes(intent));
		const bIsPreferred =
			currentUser?.activities?.includes(b.id) ||
	...
	const getActivityQuestions = (): Question[] => {
	...
	const canProceed = (): boolean => {
		...
		if (currentStep < questions.length) {
			...
			return (
				preferences[currentQuestion.id] !== undefined &&
	...
	const handleSelectOption = (questionId: string, optionId: string): void => {
		setPreferences((prev) => ({
			...prev,
			[questionId]: optionId,
		}));
	...
	const handleSubmit = async (): Promise<void> => {
		...
		try {
			const peopleNeededNum = peopleNeeded
				? parseInt(peopleNeeded, 10)
				: undefined;
	...
	const isActivityActive = (activityId: string): boolean => {
		return (
			userActiveSearches?.some(
				(search) => search.activityType === activityId,
			) ?? false
		);
	};
	...
	const handlePeopleNeededChange = (e: ChangeEvent<HTMLInputElement>): void => {
	...
	const progressSteps = (totalSteps: number): number[] =>
		Array.from({ length: totalSteps }, (_, index) => index);

	const renderModalContent = (): React.ReactElement => {
		if (showSuccess) {
			const hasMatches = matchResult && matchResult.requestsSent > 0;
			return (
					...
					{hasMatches ? (
						<>
							<h3 className="text-xl font-semibold text-white mb-2">
								{matchResult.requestsSent} Match
								{matchResult.requestsSent > 1 ? "es" : ""} Found!
							</h3>
						...
						<>
							...
							<p className="text-gray-400">
								{matchResult?.matchesFound && matchResult.matchesFound > 0
			...
			return (
				<>
					...
					<div className="flex items-center gap-1 mb-6">
						{progressSteps(totalSteps).map((i) => (
							<div
								key={`people-step-${i}`}
						...
						{peopleNeeded && (
							<div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-purple-500/20 text-center">
								<span className="text-purple-300 font-medium">
									Looking for {peopleNeeded}{" "}
									{parseInt(peopleNeeded, 10) === 1 ? "partner" : "partners"}
								</span>
								<p className="text-xs text-gray-400 mt-1">
									Your search will auto-close when all spots are filled
								</p>
							</div>
						)}
						{!peopleNeeded && (
							<div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10 text-center">
								...
								<p className="text-xs text-gray-500 mt-1">
									Your search will stay open until you close it
								</p>
							</div>
						)}
					</div>
			...
			<main className="max-w-6xl mx-auto px-4 pt-20 pb-24">
				...
				<header className="relative mb-8">
					...
					<div className="relative">
						...
						<p className="text-gray-400">
							Find the perfect partner for any activity
						</p>
					</div>
				</header>
						...
						{sortedActivities.map((activity) => {
							...
							return (
								...
								>
									...
									<div
										className={`absolute inset-0 bg-gradient-to-br ${activity.gradient} opacity-0 group-hover:opacity-10 transition-opacity`}
									/>
				...
				<section className="mb-8">
					<div className="flex items-center justify-between mb-4">
						...
						<span className="text-sm text-gray-500">
							{sortedCommunities.length} communities
						</span>
					</div>
					...
					{sortedCommunities.length > 0 ? (
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
							{sortedCommunities.map((community) => {
								...
								const isUserInterest = currentUser?.intents?.includes(
									community.category ?? "",
								);
